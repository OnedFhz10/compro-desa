                    ...
                    @forelse($data as $item)
                        <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
                            <div class="flex justify-between items-start mb-3">
                                <div>
                                    <span class="bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider">
                                        {{ $item->year }}
                                    </span>
                            ...
                            @if ($item->file_path)
                                <a href="{{ asset('storage/' . $item->file_path) }}" target="_blank" 
                                   class="flex items-center justify-center gap-2 w-full bg-blue-600 active:bg-blue-700 text-white py-2.5 rounded-lg text-sm font-bold transition shadow-lg shadow-blue-600/20">
                            ...
                            @forelse($data as $key => $item)
                                <tr class="hover:bg-slate-50/50 transition duration-150">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-purple-600 font-bold uppercase mt-1">Laporan {{ $item->year }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if ($item->file_path)
                                            <a href="{{ asset('storage/' . $item->file_path) }}" target="_blank" 
                                               class="inline-flex items-center space-x-2 bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 hover:border-blue-300 px-4 py-2 rounded-lg text-sm font-bold transition shadow-sm">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                                                <span>Download</span>
                                            </a>
                                        @else
